...
- Cowork appliance Phase 2.5 — remote-backed storage: `appliance/storage.sh` mounts a member's Google Drive / OneDrive / Dropbox at `~/CloudDrives/<name>` via rclone systemd user units with a bounded VFS cache (default 10G), so Cowork/Code project folders live in the provider and the appliance disk only holds hot data. One-time OAuth via `rclone authorize` paste (token through env and the member's own rclone.conf, never argv); `list`/`remove` manage mounts without touching provider data.
- Cowork appliance Phase 1.5 — zero-touch tunnel provisioning: `setup.sh --cf-api-token-file FILE --access-allow LIST` creates the remotely-managed Cloudflare tunnel, ingress config, proxied DNS CNAME, and the Access application + allow policy entirely via the API (scoped token, read from a file, never argv) and installs the connector — bare OS to an Access-protected URL in one command, no interactive `cloudflared tunnel login`. `member.sh` add/remove updates remote ingress, member DNS, and per-member Access apps in api mode; the doctor recognizes the remotely-managed shape. Test-environment tiers (x86 VPS → N100 mini PC → Pi 5) are now specified in `docs/cowork-appliance-phases.md`.
- Cowork appliance (Phase 1–5): a headless, team-accessible Claude Desktop server layer under `appliance/` — `setup.sh` provisioning with engine auto-selection (official apt build with KVM, or this repo's build with the bwrap Cowork backend), kasmVNC/xrdp/Tailscale session profiles behind a cloudflared tunnel skeleton, `member.sh` multi-user lifecycle with systemd slice quotas and idempotent ingress edits, an `appliance-doctor` (fails loudly on any session port bound beyond loopback), two dependency-free test-bench MCP servers (`desktop-control` nested-display GUI automation, experimental `vm-bench` QEMU targets), a managed-settings `sshConfigs` generator, cloud-init bootstrap, an admin runbook, and 87 BATS tests with a live Xvfb end-to-end in a dedicated `appliance-tests.yml` workflow. Design and phase specs in `docs/cowork-appliance-design.md` / `docs/cowork-appliance-phases.md`.

### Fixed

- Cowork Patches 1b (VM-supported evaluator) and 6 step 1 (ENOENT→ECONNREFUSED retry expansion) apply again on Claude Desktop 1.18286.0 — upstream unified the win32-only capability probe into a cross-platform `process.platform` evaluator and rewrote the retry loop's ENOENT check as a guarded `instanceof Error` ternary, breaking both anchors and failing the build's verify-patches gate (2/14 markers missing). Both patches now try the new shape first with the old shape as fallback, the injected ECONNREFUSED arm reuses upstream's own throwable guard, and the marker patterns cover both generations. Verified 14/14 markers against the real 1.18286.0 bundle.
